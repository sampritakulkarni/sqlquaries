...
-- 
select Book_name,count(book_name) as No_of_Copies from Books_details1 group by Book_name;

select publish_year, max(Book_price) as Hightest_cost_of_Book from books_details group by publish_year;

select publish_year, min(Book_price) as lowest_cost_of_book from books_details1 group by publish_year;

select Author_name, publish_year,count(book_name) as No_of_Books_in_year_by_author from books_details1 group by Author_name, publish_year;

select publish_year,min(book_price) as lowest_cost_of_book, max(book_price) as highest_cost_of_book from books_details group by publish_year;

select publish_year, sum(book_price) as total_sum_of_book_in_year from books_details group by publish_year;

select publish_year,sum(book_price) as total_sum_of_books_in_year from books_details group by publish_year order by publish_year;

--  Having--it is used to filter the data , on already grouped data

select publish_year,sum(book_price) as total_sum_of_book_in_year from books_details group by publish_year having total_sum_of_book_in_year>'2017';

select book_volume,count(book_name) as no_of_book_in_volume from books_details1 group by book_volume having book_volume>6;

select author_name , min(book_price) as lowest_book_price_by_author from books_details1 group by author_name having lowest_book_price_by_author < 495; 

select * from books_details ;

-- Rollback : it will undo the transaction from database. if the transaction is not commited

set autocommit=0;
delete from Books_Details1 where Book_id=4;

rollback;
delete from book1_details1 where book_id=7;
